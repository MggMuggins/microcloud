...
func (s OVNService) Bootstrap(ctx context.Context) error {
	err := s.m.NewCluster(s.name, util.CanonicalNetworkAddress(s.address, int64(s.port)), nil, 2*time.Minute)
	if err != nil {
		return err
	}
...
func (s OVNService) Join(ctx context.Context, joinConfig JoinConfig) error {
	return s.m.JoinCluster(s.name, util.CanonicalNetworkAddress(s.address, int64(s.port)), joinConfig.Token, nil, 5*time.Minute)
}
